...
int arrCounter = 0;
float combCoord[5] = {0,0,0,0,0};
uint8_t locChangeThreshold = .0001;

float calculateAvgCoord(void) {
    combCoord[arrCounter] = gpsLat->getValue()+gpsLon->getValue();
    float coordAvg = 0;
    float coordSum = 0;
    float perChange = 0;
    int valsPresent = 0;
    //comparing against pervious three GPS measurements
    for(int j=0; j<5; j++) {
        //checking for error value
        if (combCoord[j]/2 != 0){
            coordSum += combCoord[j];
            valsPresent ++;
        }
    }
    /*check how many valid values we have
    ** also check if previous measurement has valid value
    */
    if (valsPresent != 0 && combCoord[5] != 0){
        coordAvg = coordSum/valsPresent;
        perChange = (coordAvg - combCoord[5])/coordAvg;
    }
    return perChange;
}

const uint8_t calculatedLocationChangeResolution = 7;
const char* calculatedLocationChangeName = "locationChange";
const char* calculatedLocationChangeUnit = "percent";
const char* calculatedLocationChangeCode = "locChange";

Variable* calculatedLocationChange = new Variable(
    calculateAvgCoord, calculatedLocationChangeResolution, calculatedLocationChangeName,
    calculatedLocationChangeUnit, calculatedLocationChangeCode);
...
Variable* variableList[] = {
    ...
    new Variable(calculateAvgCoord, calculatedLocationChangeResolution, calculatedLocationChangeName,
    calculatedLocationChangeUnit, calculatedLocationChangeCode),
...
void loop() {
    ...
    if (getBatteryVoltage() < 2.4) {
        ...
        */
        ...
        if (locChangeThreshold > calculatedLocationChange->getValue()){
            takePhotos();
            Serial.println("GPS Coordinates are stationary");
    }
